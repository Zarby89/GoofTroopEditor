...
public unsafe class PointeredImage
{
    ...
    public void Draw8bppTiles(int x, int y, byte[] tiles8bpp, int widthTiles, byte mx, byte my, byte rightside = 0)
    {
        int tileXPos = 0;
        int tileYPos = 0;
        int tileId = 0;
        ...
        for (int i = 0; i < tiles8bpp.Length / 0x40; i++)
        {
            for (int yl = 0; yl < 8; yl++)
            {
                for (int xl = 0; xl < 8; xl++)
                {
                    ...
                    this[xx + x, yy + y] = (byte)(tiles8bpp[(tileId * 0x40) + xl + (yl * 8)] + rightside);
                    //}
                }
            }
            tileId++;
    ...
    public void Destroy()
    {
        Marshal.FreeHGlobal(indexedptr);
    }

    public void DrawTilemap(ushort[] tilemap, int width, PointeredImage source, int startingIndex = 0, int nbrTiles = -1)
    {
        if (nbrTiles == -1) { nbrTiles = tilemap.Length; }
        for (int i = 0; i < nbrTiles; i++)
        {
            int x = (i % width) * 8;
            int y = (i / width) * 8;

            DrawBitmapTile(x, y, source, tilemap[i + startingIndex]);

        }
    }

    public void DrawBitmapTile(int x, int y, PointeredImage source, ushort tile, bool spr = false, int w = 8, int h = 8)
    {
        byte bppLength = 16;
    ...
    public void ClearRectangle(byte clear, Rectangle rect)
    {
